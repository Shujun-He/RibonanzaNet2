...
for i in tqdm(sorted_indices):
    try:
        file_name_csv = f"test_results/config_{i}.yaml_casp16_ribonanzanet.csv"
...
for i in tqdm(sorted_indices):
    try:
        ...
        total_score_parquet = (
            avg_F1_parquet + avg_CP_F1_parquet
...
if len(average_F1s_parquet) > 0:
    ...
    overall_avg_CP_F1_parquet = sum(average_CP_F1s_parquet) / len(
        average_CP_F1s_parquet
    )
else:
...
if overall_avg_F1_csv is not None and overall_avg_CP_F1_csv is not None:
    ...
    axes[0, 0].text(
        2,
        overall_avg_CP_F1_csv,
        f"Avg: {overall_avg_CP_F1_csv:.3f}",
        ha="center",
        va="bottom",
        color="blue",
    )
...
if overall_avg_F1_parquet is not None and overall_avg_CP_F1_parquet is not None:
    axes[1, 0].text(
        1,
        overall_avg_F1_parquet,
        f"Avg: {overall_avg_F1_parquet:.3f}",
        ha="center",
        va="bottom",
        color="green",
    )
    axes[1, 0].text(
        2,
        overall_avg_CP_F1_parquet,
        f"Avg: {overall_avg_CP_F1_parquet:.3f}",
        ha="center",
        va="bottom",
        color="green",
    )
...
axes[1, 1].scatter(
    average_F1s_parquet, average_CP_F1s_parquet, color="orange", alpha=0.7
)
