...
folders = [
    "../../exps/test10",
    "/lustre/fs0/scratch/shujun/8m_exps/test4_init_fromrn1_pl_bs1",
    ".",
]
labels = ["Rnet1\n210k data", "Rnet2\n4m data", "Rnet2\n30m data"]
all_folder_metrics = {}
...
for folder in folders:
    ...
    casp15_F1s, casp15_CP_F1s = process_files(
        folder + "/test_results/config_{}.yaml_casp15_ribonanzanet.csv", sorted_indices
    )
    casp16_F1s, casp16_CP_F1s = process_files(
        folder + "/test_results/config_{}.yaml_casp16_ribonanzanet.csv", sorted_indices
    )
    pdb_F1s, pdb_CP_F1s = process_files(
        folder + "/test_results/config_{}.yaml_finetuned_test.parquet", sorted_indices
    )
...
for i, category in enumerate(categories):
    ...
    axes[i, 0].axhline(
        reference_F1_scores[category], color="red", linestyle="--", label="Rnet1 F1"
    )
    ...
    axes[i, 1].axhline(
        reference_CP_F1_scores[category],
        color="red",
        linestyle="--",
        label="Rnet1 CP_F1",
    )
...
plt.savefig("compare_w_baseline_nvidia.png", dpi=300)
plt.show()
