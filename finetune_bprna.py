...
parser = argparse.ArgumentParser(
    description="Train model with YAML configuration file."
)
...
train_split = train_data[train_data["publication_date"] < val_cutoff].reset_index(
    drop=True
)
val_split = train_data[train_data["publication_date"] >= val_cutoff].reset_index(
    drop=True
)
...
class finetuned_RibonanzaNet(RibonanzaNet):
    def __init__(self, config, pretrained=False):
        ...
        self.ct_predictor = nn.Sequential(
            ...
            nn.Linear(
                config.pairwise_dimension, 1
            ),  # Linear layer with output dimension of 1
        )
...
schedule = torch.optim.lr_scheduler.CosineAnnealingLR(
    optimizer, T_max=(epochs - 2) * len(train_loader) // batch_size
)
...
model, optimizer, train_loader, val_loader, bp_loader, schedule = accelerator.prepare(
...
for epoch in range(epochs):
    ...
    for idx, batch in enumerate(tbar):
        ...
        with accelerator.autocast():
            ...
            loss = (
                loss.mean() * sequence.shape[1] ** loss_power_scale / normalize_length
            )
...
for i in range(len(test_preds)):
    s, bp = _hungarian(
        mask_diagonal(test_preds[i][0]), theta=best_theta, min_len_helix=1
    )
    ct_matrix = np.zeros((len(s), len(s)))
    for b in bp:
        ct_matrix[b[0], b[1]] = 1
...
for i in range(len(ribonanza_casp_ss)):
    s, bp = _hungarian(
        mask_diagonal(ribonanza_casp_ss[i][0]), theta=best_theta, min_len_helix=1
    )
