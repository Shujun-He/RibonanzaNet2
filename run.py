...
if hasattr(config, "dataset2drop"):
    ...
    train_indices = dataset_dropout(
        data_dict["dataset_name"], train_indices, config.dataset2drop
    )
...
train_dataset = RNADataset(
    hdf_files, train_indices, flip=config.use_flip_aug, add_noise=config.use_noise_aug
)
...
lr_schedule = torch.optim.lr_scheduler.CosineAnnealingLR(
    optimizer, len(train_loader) // config.gradient_accumulation_steps
)
...
# exit()
optimizer, train_loader, val_loader, lr_schedule = accelerator.prepare(
    optimizer, train_loader, val_loader, lr_schedule
)
...
for epoch in range(config.epochs):
    ...
    for idx, batch in enumerate(tbar):
        ...
        with accelerator.autocast():
            ...
            loss = (
                loss * SN[:, None, :].clip(0.5, 1)
            )  # weight with SN, downweight low quality data, and high quality data has up to 1 weight
        ...
        if (idx + 1) % config.gradient_accumulation_steps == 0:
            ...
            if total_steps % config.log_interval == 0:
                accelerator.save_state(
                    f"models/step_{total_steps}", safe_serialization=False
                )
