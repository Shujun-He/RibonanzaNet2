...
pl_train = pl.read_parquet(
    f"{config.input_dir}/merged_noisy_train_test_3parts/top3_ensembled_unused_w_test5.parquet"
)
pl_test = pl.read_parquet(
    f"{config.input_dir}/merged_noisy_train_test_3parts/pl_preds_top3_w_ribonanzanet.parquet"
)
...
pl_data = pl_data.with_columns(pl.lit(10.0).alias(f"signal_to_noise"))
...
# save labels
filename = (
    "pl_data/labels.mmap"  # Specify the path where the memmap file should be stored
)
...
# save errors
filename = (
    "pl_data/errors.mmap"  # Specify the path where the memmap file should be stored
)
dtype = "float32"  # Change this to match the dtype of your labels array
mode = "w+"  # Write mode, will create or overwrite existing file
