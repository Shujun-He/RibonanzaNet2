...
data = pd.read_parquet(
    "../david_NAKB_dec/PDB_ATOM1_train_RNA_degformer_embeddings.parquet"
)
...
for i, (train_index, val_index) in enumerate(
    kf.split(train_data, train_data["length"] > 100)
):
    break
    ...
    def __init__(self, config, pretrained=False):
        ...
        if pretrained:
            self.load_state_dict(
                torch.load("models/epoch_14/pytorch_model_fsdp.bin", map_location="cpu")
            )
