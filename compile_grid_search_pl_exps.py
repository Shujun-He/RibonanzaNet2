...
if __name__ == "__main__":
    ...
    exp_dirs = ['rnet2_plinit',
                'rn2bcde_02M', 'rn2bcde_04M', 'rn2bcde_12M', 'rn2bcde_24M',
                'rn2fgh_02M', 'rn2fgh_04M', 'rn2fgh_12M', 'rn2fgh_24M',
                'rn2_random_02M', 'rn2_random_04M', 'rn2_random_12M', 'rn2_random_24M',
                'rnet2alpha_baseline']

    all_results = []
    for exp_dir in exp_dirs:
        results = process(range(1, n + 1),
                          f"{base_dir}/{exp_dir}/grid_search_scores/config_{{}}.yaml.txt",
                          f"{base_dir}/{exp_dir}/test_results/config_{{}}.yaml_casp15_ribonanzanet.csv",
                          f"{base_dir}/{exp_dir}/test_results/config_{{}}.yaml_finetuned_test.parquet")
    ...
    plot_data(axes[0, 0],
              ...
              exp_dirs,
              'PDB Data: Average F1 Scores', colors, (0.85, 1))
    plot_data(axes[0, 1],
              avg_CP_F1s_parquet,
              [np.mean(ff) for ff in avg_CP_F1s_parquet],
              exp_dirs,
              'PDB Data: Average CP F1 Scores', colors, (0.4, 1))
    plot_data(axes[1, 0],
              avg_F1s_csv,
              [np.mean(ff) for ff in avg_F1s_csv],
              exp_dirs,
              'CASP15 Data: Average F1 Scores', colors, (0.85, 1))
    plot_data(axes[1, 1],
              avg_CP_F1s_csv,
              [np.mean(ff) for ff in avg_CP_F1s_csv],
              exp_dirs,
              'CASP15 Data: Average CP F1 Scores', colors, (0.4, 1))

    plt.tight_layout()
    plt.savefig("combined_pl_exps_plots.pdf")
    plt.savefig("combined_pl_exps_plots.png")
