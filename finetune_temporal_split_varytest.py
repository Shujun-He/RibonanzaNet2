...
parser.add_argument(
    ...
    type=int,
    required=True,
    help="number of samples to use as test"
)
...
data_index = np.argsort(data['publication_date'].values, kind='stable')

train_data = data.iloc[data_index[:-args.n_test]].reset_index(drop=True)
test_data = data.iloc[data_index[-args.n_test:]].reset_index(drop=True)
test_data.shape

#use 2 years data as val
val_cutoff = pd.Timestamp('2018-05-01')

total_sz = train_data.shape[0]
val_sz = np.round(0.2 * total_sz).astype(int)
train_split = train_data.iloc[:-val_sz].reset_index(drop=True)
val_split = train_data.iloc[-val_sz:].reset_index(drop=True)
...
class finetuned_RibonanzaNet(RibonanzaNet):
    def __init__(self, config, pretrained=False):
        ...
        if pretrained:
            self.load_state_dict(torch.load(config.best_weights_path,map_location='cpu'))
...
for epoch in range(epochs):
    ...
    if f1 > best_f1:
        ...
        prefix = args.config.split('/')[-1] + f'_{args.n_test:03d}'

        torch.save(best_state_dict, f"gs_weights/{prefix}_best_weights.pth")

        with open(f"gs_scores/{prefix}.txt",'w+') as f:
            f.write(str(best_f1))
...
os.system('mkdir test2_results')
test_data.to_parquet(f"test2_results/{prefix}_finetuned_test.parquet",index=False)
...
casp_data.to_csv(f"test2_results/{prefix}_casp15_ribonanzanet.csv",index=False)
...
casp16_targets.to_csv(f"test2_results/{prefix}_casp16_ribonanzanet.csv",index=False)
