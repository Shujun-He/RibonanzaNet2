...
def process(config_indices, score_file_pattern, csv_pattern, parquet_pattern, additional_csv_pattern=None):
    ...
    print(f"Best validation Parquet configuration: {sorted_indices[0]}")
    print(f"  Average F1 Score: {avg_F1s_parquet[0]:.3f}")
    print(f"  Average CP F1 Score: {avg_CP_F1s_parquet[0]:.3f}")
...
if __name__ == "__main__":
    ...
    exp_dirs = ['/nrs/das/rnastruct/roi_exps/rnet2_plinit',
                ...
                # '/groups/flyem/home/huangg/proj/rnaxjanelia/RibonanzaNet2-ft/rnet2alpha_rep_bs6_opt_mid']
    ...
    for exp_dir in exp_dirs:
        results = process(range(1, n + 1),
                          f"{exp_dir}/grid_search_scores/config_{{}}.yaml.txt",
                          f"{exp_dir}/test_results/config_{{}}.yaml_casp15_ribonanzanet.csv",
                          f"{exp_dir}/test_results/config_{{}}.yaml_finetuned_test.parquet")
    ...
    plt.tight_layout()
    plt.savefig("rep_combined_crossexps_plots.pdf")
    plt.savefig("rep_combined_crossexps_plots.png")
