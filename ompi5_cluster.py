...
class OmpiCluster(clusters.ClusterEnv):
    ...
    def is_env_present(cls) -> bool:
        return _PROCESS_ID in os.environ and (
            _ORTE_URI in os.environ or _PMIX_URI in os.environ
        )

    @classmethod
    def get_coordinator_address(cls, timeout_secs: int | None) -> str:
        if _ORTE_URI in os.environ:
            ...
            # Examples of orte_uri
            # prterun-%{hostname}-%{num_job_id}@0.0;tcp4://%{ip_v4_addr}:%{port}
            orte_uri = os.environ[_PMIX_URI]

        # Pick port in ephemeral range [(65535 - 2^12 + 1), 65535]
        job_id = int(hashlib.sha256(orte_uri.encode("utf-8")).hexdigest(), 16) % 2**12
        port = job_id % 2**12 + (65535 - 2**12 + 1)
        launcher_ip_match = re.search(
            r"tcp4?://(.+?)[,:]|tcp6://\[(.+?)[,\]]", orte_uri
        )
        if launcher_ip_match is None:
            raise RuntimeError(
                "Could not parse coordinator IP address from Open MPI environment."
            )
