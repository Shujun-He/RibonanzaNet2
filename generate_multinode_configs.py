...
def main():
    parser = argparse.ArgumentParser(
        description="Generate distributed training configs"
    )
    parser.add_argument(
        "--node_list",
        type=str,
    ...
    parser.add_argument("--master_node", type=str, help="master_node")
    parser.add_argument(
        "--n_nodes",
        type=int,
        default=None,
    ...
    parser.add_argument(
        "--n_gpus_per_node", type=int, default=8, help="Number of GPUs per node"
    )
    parser.add_argument(
        "--python_env_path",
        type=str,
        default="/lustre/fs0/scratch/shujun/miniconda3/envs/torch",
        help="Path to Python environment (e.g., /lustre/fs0/scratch/user/miniconda3/envs/torch)",
    )
    parser.add_argument(
        "--config_dir",
        type=str,
        default="accelerate_configs",
        help="Directory containing accelerate config files",
    )
    parser.add_argument(
        "--output_dir", type=str, default=".", help="Directory to save job scripts"
    )
    parser.add_argument(
        "--script_name", type=str, default="run.py", help="Name of the training script"
    )
    parser.add_argument(
        "--partition", type=str, default="defq", help="SLURM partition to use"
    )
    parser.add_argument(
        "--job_name", type=str, default="distributed-job", help="Base name for the job"
    )
