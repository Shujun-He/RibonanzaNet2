...
def process_and_plot(config_indices, score_file_pattern, csv_pattern, parquet_pattern, additional_csv_pattern=None):
    scores = []
    ...
    # Middle row: Parquet
    plot_data(axes[0, 0], [avg_F1s_parquet, avg_CP_F1s_parquet],
              [np.mean(avg_F1s_parquet), np.mean(avg_CP_F1s_parquet)],
              ['Average F1', 'Average CP F1'],
              'PDB Data: Boxplot of Average F1 and CP F1 Scores', 'lightgreen')
    scatter_data(axes[0, 1], avg_F1s_parquet, avg_CP_F1s_parquet,
                 'PDB Data: Scatter Plot of Average F1 vs CP F1',
                 'Average F1', 'Average CP F1', 'orange')

    plot_data(axes[1, 0], [avg_F1s_csv, avg_CP_F1s_csv],
              [np.mean(avg_F1s_csv), np.mean(avg_CP_F1s_csv)],
              ['Average F1', 'Average CP F1'],
              'CASP15 Data: Boxplot of Average F1 and CP F1 Scores', 'lightblue')
    scatter_data(axes[1, 1], avg_F1s_csv, avg_CP_F1s_csv,
                 'CASP15 Data: Scatter Plot of Average F1 vs CP F1',
                 'Average F1', 'Average CP F1', 'purple')

    # Bottom row: Additional CSV
    if additional_csv_pattern:
        plot_data(axes[2, 0], [avg_F1s_additional_csv, avg_CP_F1s_additional_csv],
                  [np.mean(avg_F1s_additional_csv), np.mean(avg_CP_F1s_additional_csv)],
                  ['Average F1', 'Average CP F1'],
                  'CASP16: Boxplot of Average F1 and CP F1 Scores', 'lightcoral')
        scatter_data(axes[2, 1], avg_F1s_additional_csv, avg_CP_F1s_additional_csv,
                     'CASP16: Scatter Plot of Average F1 vs CP F1',
                     'Average F1', 'Average CP F1', 'red')
